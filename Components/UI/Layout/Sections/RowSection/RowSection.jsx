...
export default function RowSection({
  ...
  const gridTemplateColumns = imageAlignment === "left" ? "1fr  1fr" : "1fr 1fr"; 
  return (
    <section className={`${styles.section}`} id="services" style={{background: backgroundColor ? backgroundColor : null}}>
      <Container maxWidth="xl">
        <div className={`${styles.wrapper} `} style={{gridTemplateColumns: gridTemplateColumns}}>
          <div
            className={`${styles.contentWrapper} `}
            style={{ gridColumn: contentAlignment }}
          >
            <Typography variant="h6" component="div" className={`${styles.subtitle}`} sx={{color: !fontColor ? "var(--light-on-surface)" : fontColor }}>
              {subtitle}
            </Typography>
            <Typography variant="h3" component="h2" className={`${styles.title}`} sx={{color: !fontColor ? "var(--light-on-surface)" : fontColor }}>
              {title}
            </Typography>
                ...
                return (
                  ...
                  >
                    ...
                    <Typography
                      variant="subtitle1"
                      component="span"
                      color={`${fontColor ? "var(--dark-on-surface)" : "var(--dark-on-surface)"  }`}
      ...
      <CustomAccordion qaData = {accordionData}  /> 
            ...
            {ctaGroup?.cta && (
              <Link href={ctaGroup.cta.url} className="cta mt-16 inline-block">
                <Button variant={ctaGroup.cta_type} >
                  {ctaGroup.cta.title}
                </Button>
              </Link>
            )}
